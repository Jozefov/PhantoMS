...
class SkipConnectionRetrievalHead(nn.Module):
    def __init__(self, input_size: int,
        ...
        # Final layers
        self.fc_final = nn.Linear(hidden_size, hidden_size // 2)
        self.bn_final = nn.BatchNorm1d(hidden_size // 2)
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.dropout(x)
        out = self.output_layer(x)  # No activation since we'll use BCEWithLogitsLoss
